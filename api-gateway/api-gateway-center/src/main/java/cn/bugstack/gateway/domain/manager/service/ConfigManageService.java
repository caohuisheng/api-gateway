...
public class ConfigManageService implements IConfigManageService {
    ...
    public void distributionGatewayServerNode(String groupId, String gatewayId, String systemId) {
        ApplicationSystem applicationSystem = configManageRepository.queryApplicationSystemById(systemId);
        if(null == applicationSystem) throw new RuntimeException("网关算力与系统节点挂载失败，systemId:" + systemId);
        configManageRepository.distributionGatewayServerNode(groupId, gatewayId, systemId, applicationSystem.getSystemName());
    }
}
