...
public class ProtocolDataHandler extends BaseHandler<FullHttpRequest> {
    ...
    protected void session(ChannelHandlerContext ctx, Channel channel, FullHttpRequest request) {
        ...
        try {
            ...
            e.printStackTrace();
            DefaultHttpResponse response = new ResponseParser().parse(GatewayResultMessage.buildError(AgreementConstants.ResponseCode._502.getCode(), "网关协议调用失败",configuration.getNode()));
            channel.writeAndFlush(response);
        }
    }
