...
public class AuthorizationHandler extends BaseHandler<FullHttpRequest> {
    ...
    protected void session(ChannelHandlerContext ctx, Channel channel, FullHttpRequest request) {
        ...
        try {
            ...
            if(httpStatement.isAuth()){
                ...
                if(token == null || token.equals("")){
                    DefaultFullHttpResponse response = new ResponseParser().parse(
                            GatewayResultMessage.buildError(AgreementConstants.ResponseCode._403.getCode(),"token不能为空",configuration.getNode()));
                ...
                if(status){
                    ...
                    DefaultFullHttpResponse response = new ResponseParser().parse(
                            GatewayResultMessage.buildError(AgreementConstants.ResponseCode._403.getCode(),"您无权访问此接口",configuration.getNode()));
                    channel.writeAndFlush(response);
                }
            }else{
                //不需要鉴权，直接放行
            ...
            e.printStackTrace();
            DefaultFullHttpResponse response = new ResponseParser().parse(
                    GatewayResultMessage.buildError(AgreementConstants.ResponseCode._500.getCode(),"服务器鉴权异常",configuration.getNode()));
            channel.writeAndFlush(response);
        }
    }
}
