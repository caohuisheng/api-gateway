...
public class GatewayCenterService {
    ...
    public void doRegisterApplication(String address, String systemId, String systemName, String systemType, String systemRegistry){
        ...
        try {
            resultStr = HttpUtil.post(address + "/wg/admin/register/registerApplication", paramMap, 500);
        } catch (Exception e) {
            log.error("应用服务注册异常，链接资源不可用：{}", address + "/wg/admin/register/registerApplication", e);
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
        log.info("像网关中心注册应用服务 systemId:{} systemName:{} 注册结果：{}", systemId, systemName, resultStr);
    ...
    public void doRegisterApplicationInterface(String address, String systemId, String interfaceId, String interfaceName, String interfaceVersion){
        ...
        String resultStr = null;
        try {
            resultStr = HttpUtil.post(address + "/wg/admin/register/registerApplicationInterface",paramMap,500);
        } catch (Exception e) {
            log.error("应用服务接口注册异常，链接资源不可用：{}", address + "/wg/admin/register/registerApplicationInterface", e);
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
        log.info("向网关注册应用接口服务 systemId:{} interfaceId:{} interfaceName:{}, 注册结果：{}", systemId, interfaceId, interfaceName, resultStr);
        if(!"0000".equals(result.getCode()) && !"0003".equals(result.getCode())){
            throw new GatewayException("像网关中心注册应用接口服务异常 systemId:" + systemId + " interfaceId:" + interfaceId);
        }
    }

    public void doRegisterApplicationInterfaceMethod(String address,String systemId,String interfaceId,String methodId,String methodName,
        ...
        String resultStr = null;
        try {
            resultStr = HttpUtil.post(address + "/wg/admin/register/registerApplicationInterfaceMethod",paramMap,500);
        } catch (Exception e) {
        ...
        if(!"0000".equals(result.getCode()) && !"0003".equals(result.getCode())){
            throw new GatewayException(String.format("向网关中心注册应用接口方法服务异常 systemId:%s interfaceId:%s methodId:%s",systemId,interfaceId,methodId));
        }
    }
    ...
    public void doRegisterEvent(String address,String systemId){
        ...
        if(!"0000".equals(result.getCode()) && !"0003".equals(result.getCode())){
            throw new GatewayException("向网关中心注册事件异常 systemId:" + systemId);
        }
    }
